...
if __name__ == "__main__":
    ...
    experiment.set_name(f"{HOLDOUT_SET}_seed_{SEED}")
    experiment.log_parameters(flags)

    # Creating the output directory and the experiment directory
    EXPERIMENT_DIR = os.path.join(OUTPUT_DIR, f"{HOLDOUT_SET}_seed_{SEED}")
    ...
    for indexer, label in enumerate(["low"]):
        ...
        optimizer_ft = optim.SGD(
            model_ft.parameters(), lr=0.001, weight_decay=0.001, momentum=0.9
        )
