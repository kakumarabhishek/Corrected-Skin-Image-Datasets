...
from medmnist_corrected.info import INFO, DEFAULT_ROOT


def available():
    '''List all available datasets.'''
    print(f"MedMNIST v{medmnist_corrected.__version__} @ {medmnist_corrected.HOMEPAGE}")
...
def download(root=DEFAULT_ROOT):
    ...
    for key in INFO.keys():
        ...
        _ = getattr(medmnist_corrected, INFO[key]['python_class'])(
...
def save(flag, folder, postfix="png", root=DEFAULT_ROOT):
    ...
    for split in ["train", "val", "test"]:
        ...
        dataset = getattr(medmnist_corrected, INFO[flag]['python_class'])(
...
def evaluate(path):
    ...
    '''
    medmnist_corrected.Evaluator.parse_and_evaluate(path)
...
def test(save_folder="tmp/", root=DEFAULT_ROOT):
    ...
    for key in INFO.keys():
        ...
        for split in ["train", "val", "test"]:
            ...
            dataset = getattr(medmnist_corrected, INFO[key]['python_class'])(
                split=split, root=root)
            assert len(dataset) == INFO[key]["n_samples"][split]

            evaluator = medmnist_corrected.Evaluator(key, split)
            ...
            medmnist_corrected.Evaluator.parse_and_evaluate(
